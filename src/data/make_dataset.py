...
def export_dataset(name, views, labels):
    processed_dir = constants.DATA_DIR / "processed"
...
def _mnist(add_edge_img, dataset_class=torchvision.datasets.MNIST):
    ...
    dataset = dataset_class(root=constants.DATA_DIR / "raw", train=True, download=True, transform=transform)
...
def ccv():
    ccv_dir = constants.DATA_DIR / "raw" / "CCV"
...
def coil():
    ...
    data_dir = constants.DATA_DIR / "raw" / "COIL"
    img_size = (1, 128, 128)
    n_objs = 20
    n_imgs = 72
    n_views = 3
